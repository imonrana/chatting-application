...
const ForgotPassword = () => {
 ...
 <section className='flex flex-col gap-y-5 items-center  justify-center h-screen'>
    ...
    <h1 className='text-center text-black font-bold text-4xl'>Forgot Password</h1>
       <div className=' shadow-lg bg-lime-400 py-5 px-5 w-fit rounded'>
        <fieldset className='w-fit relative'>
                    <input 
                    onInput={handleEmailInput} 
                    onChange={handelEmail}
                    className='w-[372px] h-[70px] border-b border-[rgb(3,1,76,0.20)] outline-none  peer placeholder-transparent bg-transparent' type="email" name='email' id='loginEmail' placeholder='Email Address' />
                ...
                <Button onClick={handelforgot} className="py-[10px] px-[26px] rounded font-sans font-semibold text-xl text-[#FFFFFF]">Reset</Button>
                <Link to="/login"><Button className=" mt-4 py-[10px] px-[26px] rounded font-sans font-semibold text-xl text-[#FFFFFF]"> Back to Login</Button></Link>
            </div>
    </div>
 </section>
  )
