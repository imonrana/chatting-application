...
const UserList = () => {
  ...
  useEffect(() => {
    ...
    onValue(friendRequestRef, (snapshot) => {
      ...
      snapshot.forEach((item) => {
        dataArr.push({
          combinedId: item.val().senderId + item.val().reciverId,
          frndReqId: item.key,
        });
      });
  ...
  function handelSearch(e) {
    ...
    if (data == 0) {
      ...
      userList.filter((user) => {
        const searchItem = user.username
          .toLowerCase()
        ...
        <article className="h-44 overflow-y-scroll scrollbar-thin scrollbar-thumb-primary scrollbar-track-slate-100  hover:scrollbar-thumb-red-400 active:scrollbar-thumb-red-500">
          {searchData && searchQuery.length === 0 ? (
            ...
            searchQuery.map((item, index) => (
              <div
                key={index}
                className='flex justify-between items-center px-5 pb-[13px] mb-4 relative before:content-[""] before:absolute before:bottom-0 before:left-1/2 before:-translate-x-1/2 before:bg-[rgba(0,0,0,0.25)] before:h-[1px] before:w-[277px]'
              >
                <div className="flex ">
                ...
                <div className="mt-2 px-[8px] ">
                  ...
                  blockList.includes(item.uid + data.userInfo.uid) ? (
                    <SmallButton className="px-3 py-1 cursor-default !text-base ">
                      Block
                    </SmallButton>
                  ) : friendsList.includes(data.userInfo.uid + item.uid) ||
                    friendsList.includes(item.uid + data.userInfo.uid) ? (
                    <SmallButton className="px-2 py-1 cursor-default !text-[16px]">
                      Friends
                    </SmallButton>
                    ...
                    <SmallButton
                      onClick={() => handelCancelRequest(item)}
                      className="px-2 py-1 cursor-pointer text-xs   "
                    >
                      Cancel Request
                    </SmallButton>
                  ) : friendRequestList.some(
                      (req) => req.combinedId == item.uid + data.userInfo.uid
                    ) ? (
                    <SmallButton className="px-2 py-2 cursor-default text-xs   ">
                      Pendding
                    </SmallButton>
                  ) : (
                    <SmallButton
                      onClick={() => sendFriendRequest(item)}
                      className="px-[22px] py-1 cursor-pointer"
                    >
                      +
                    </SmallButton>
                  )}
