      ...
      snapshot.forEach((item) => {
        if (
          (data.uid === item.val().msgSenderId &&
            activeData.id === item.val().msgReciverId) ||
          (data.uid === item.val().msgReciverId &&
    ...
    <div className="shadow-md rounded-[20px] pb-6">
      ...
      <header className=" mx-[52px] py-7 flex items-center justify-between border-b-2 border-[rgba(0,0,0,0.25)]">
        <div className="flex items-center gap-x-8">
          ...
          <div>
            <h2 className="font-poppins font-semibold text-2xl text-[#000000]">
              {activeData?.name || "Sewty"}{" "}
            </h2>
            <p className="font-poppins font-normal text-sm text-[rgba(0,0,0,0.85)]">
              Online
            </p>
          </div>
        </div>
